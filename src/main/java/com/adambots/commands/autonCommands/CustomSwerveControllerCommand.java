...
public class CustomSwerveControllerCommand extends CommandBase {
  ...
  public boolean isFinished() {
    ...
    double dist = Math.sqrt(Math.pow(m_pose.get().getX()-target.getX(), 2) + Math.pow(m_pose.get().getY()-target.getY(), 2));
    double rot = Math.abs(m_pose.get().getRotation().getDegrees()-target.getRotation().getDegrees());
